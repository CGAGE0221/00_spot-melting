...
plt.rcParams['font.sans-serif'] = ['PingFang SC', 'STHeiti', 'Arial Unicode MS', 'Microsoft YaHei', 'SimHei', 'Arial']
    ...
    for i in range(len(path_sequence)):
        ...
        if result:
            ...
            if dist > j_max:
                ...
                for vp in v_points:
                    result.append({'x': vp[0], 'y': vp[1], 'dwell_time': 0, 'beam_current': 0, 'is_virtual': True,
                                   'type': 'virtual'})
        result.append(curr)
    return result
...
def export_to_binary(final_path, output_filename, focus_val=0):
    ...
    for i in range(len(final_path)):
        ...
        if abs(curr_x - prev_x) > 0.0001 or abs(curr_y - prev_y) > 0.0001:
            segments.append((prev_x, prev_y, curr_x, curr_y, 0.0))
...
# 8. 可视化
# ==========================================
def visualize_result(square_size, geometry, spots, melting_spots_set, final_path, z_val, filename="result"):
    print("\n生成可视化预览...")
    ...
    ax.set_title(f'CLI 填充与轮廓点扫路径 (Z={z_val:.3f}mm)', fontsize=14, fontweight='bold')

    ax.add_patch(Rectangle((-half, -half), square_size, square_size, fill=False, edgecolor='gray', linewidth=2))
    ...
    if contour_pts:
        ax.scatter([p['x'] for p in contour_pts], [p['y'] for p in contour_pts],
                   c='orange', s=20, label=f'轮廓点 ({len(contour_pts)})', zorder=120)

    if virt_pts:
        ax.scatter([p['x'] for p in virt_pts], [p['y'] for p in virt_pts],
                   c='gray', marker='x', s=30, alpha=0.6, label=f'虚拟跳转点 ({len(virt_pts)})', zorder=80)

    all_x = [p['x'] for p in final_path]
    all_y = [p['y'] for p in final_path]
    if all_x:
        ax.plot(all_x, all_y, 'g-', alpha=0.3, linewidth=0.8, zorder=50)
        ax.plot(all_x[0], all_y[0], 'o', color='lime', markersize=15, markeredgecolor='darkgreen', markeredgewidth=2,
                label='起点', zorder=200)
        ax.plot(all_x[-1], all_y[-1], 's', color='red', markersize=12, markeredgecolor='darkred', markeredgewidth=2,
                label='终点', zorder=200)

    ax.plot([], [], 'o', color='steelblue', markersize=8, markeredgecolor='navy', label=f'填充点 ({len(infill_pts)})')
    ax.legend(loc='upper right', fontsize=9)

    info_text = (f"总点数: {len(final_path)}\n填充: {len(infill_pts)}\n轮廓: {len(contour_pts)}\n虚拟: {len(virt_pts)}")
...
def main():
    print("=" * 60)
    print("  CLI 轮廓+填充 点扫路径生成器 (基于文献实验 DoE 架构)")
    print("=" * 60)
    try:
        print("\n【步骤1】设置基板参数")
        ...
        spots = generate_substrate_spots(square_size, spot_distance, pattern_type, 0)
        ...
        print(f"\n✓ 最终总路径: {infill_count} 个填充点 + {contour_count} 个轮廓点 + {virt_count} 个虚拟点")
